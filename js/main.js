...
// We can do it in just one line *** const fifteen = inventors.filter(inventor => inventor.year >= 1500 && inventor.year <= 1600);

// Array.prototype.map()
// 2. Give us an array of the inventors' first and last names
const fullNames = inventors.map(function(invertor) {
    return invertor.first + ' ' + invertor.last;
});

// We can do it in just one line *** const fullNames = inventors.map(invertor => invertor.first + ' ' + invertor.last);
...
// We can do it in just one line *** const ordered = inventors.sort((a, b) => a.year > b.year ? 1 : -1);
